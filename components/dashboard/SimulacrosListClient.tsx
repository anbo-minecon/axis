...
import {
  ...
  AlertCircle, Loader2, Layers, BookOpen,
} from "lucide-react";
...
function SimulacroCardItem({ s }: { s: SimulacroCard }) {
  const pct    = s.resultado && s.resultado.total > 0
    ? Math.round((s.resultado.puntaje / s.resultado.total) * 100)
    : 0;
  const nivel  = s.resultado ? getNivel(pct) : null;

  const materiaLabel = s.tieneSesiones ? "Multi-materia" : s.materia;

  // Imagen: para multi-materia usar default
  const Imagen = () => (
    <div className="relative h-44 w-full overflow-hidden bg-gradient-to-br from-blue-900/30 to-purple-900/30">
      ...
      />
      {/* Overlay badge materia sobre imagen */}
    ...
    return (
      <div className="rounded-2xl border border-white/10 bg-[var(--bg-card)] overflow-hidden hover:border-white/20 transition group">
        ...
        <div className="p-5">
          ...
          {/* Sesiones completadas si aplica */}
          {s.tieneSesiones && s.sesiones.length > 0 && (
            <div className="flex items-center gap-1 mb-3">
              {s.sesiones.map((ses) => (
                <div key={ses.id} className="flex-1 flex items-center gap-1">
                  <div className="h-1.5 flex-1 rounded-full bg-green-500" />
                </div>
              ))}
              <span className="text-[10px] text-green-400 font-semibold ml-1">
                {s.sesiones.length}/{s.sesiones.length} sesiones ✓
              </span>
            </div>
          )}
          ...
          <div className="flex items-center gap-4 text-xs text-gray-500 mb-4">
            <span className="flex items-center gap-1"><Hash className="h-3 w-3" />{s.totalPreguntas} preguntas</span>
            <span className="flex items-center gap-1"><Clock className="h-3 w-3" />{s.resultado.tiempoUsado > 0 ? fmtTiempo(s.resultado.tiempoUsado) : `${s.tiempoMin} min`}</span>
          </div>
          ...
          <div className="rounded-xl bg-white/5 border border-white/10 px-4 py-3 mb-3">
            <div className="flex items-center justify-between mb-1">
              <p className="text-xs text-gray-500">
                {s.resultado.estadoCalif === "OFICIAL" ? "Puntaje TRI oficial" : "Puntaje preliminar"}
              </p>
              {s.resultado.estadoCalif === "OFICIAL" && (
                <span className="text-[10px] font-bold text-violet-400 rounded-full bg-violet-500/20 px-1.5 py-0.5">TRI</span>
              )}
            </div>
            <div className="flex items-end justify-between">
              <span className="text-2xl font-extrabold text-[var(--text-primary)]">
                {s.resultado.estadoCalif === "OFICIAL" && s.resultado.puntajeTRI != null
                  ? Math.round(s.resultado.puntajeTRI)
                  : s.resultado.puntaje}
  ...
  return (
    <div className="rounded-2xl border border-white/10 bg-[var(--bg-card)] overflow-hidden hover:border-blue-500/40 hover:shadow-lg hover:shadow-blue-500/5 transition group">
      ...
      <div className="p-5">
        ...
        {s.fechaCierre && (
          <p className="text-xs text-amber-500 mb-3">Cierra: {fmtFecha(s.fechaCierre)}</p>
        )}
