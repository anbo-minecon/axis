// app/api/docente/material/route.ts
...
async function verificarDocente(userId: string) {
  ...
  // Docente Y Admin tienen acceso completo a material
...
export async function GET() {
  try {
    ...
    console.error("[GET /api/docente/material]", e);
...
export async function POST(req: Request) {
  try {
    ...
    try {
      await db.auditLog.create({
        data: {
          ...
          resultado: "EXITOSO",
          mensaje:   `Material "${titulo}" creado`,
        },
      });
    } catch { /* no bloquear */ }
    ...
    console.error("[POST /api/docente/material]", e);
    return NextResponse.json({ error: "Error interno" }, { status: 500 });
  }
}