...
export default function PlanesPage() {
  ...
  const renderPlanCard = (plan: typeof planesData[0]) => {
    const borderClass = plan.popular 
      ? "border-axis-azul shadow-2xl transform md:scale-105" 
      : "border-gray-200 shadow-lg";
    
    const paddingClass = plan.popular ? "pt-16" : "";
    
    return (
      <div
        key={plan.id}
        className={`relative rounded-2xl border-2 overflow-hidden transition-all ${borderClass}`}
      >
        {plan.popular && (
        ...
        <div className={`p-8 ${paddingClass}`}>
